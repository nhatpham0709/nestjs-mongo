...
export class UserAuthController {
  ...
  async refresh(
    ...
    return {
      data: {
        expiresIn: this.configSerivce.get<number>(
  ...
  async changePassword(
    ...
    const matchPassword = hashSync(body.oldPassword, user.password)
    if (!matchPassword) {
    ...
    const newMatchPassword = hashSync(body.newPassword, user.password)
    if (newMatchPassword) {
      throw new BadRequestException({
        statusCode:
