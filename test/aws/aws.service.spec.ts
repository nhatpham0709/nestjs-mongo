...
describe('AwsS3Service', () => {
  ...
  describe('checkBucketExistence', () => {
    it('should return HeadBucketCommandOutput', async () => {
      ...
      jest.spyOn(service['s3Client'], 'send').mockResolvedValue(data as never)
    ...
    it('should throw error', async () => {
      ...
      jest.spyOn(service['s3Client'], 'send').mockRejectedValue(error as never)
  ...
  describe('putItemInBucket', () => {
    it('should accept path and return correct result', async () => {
      ...
      const result = await service.putItemInBucket('file.png', content, options)
  ...
  describe('deleteFolder', () => {
    it('deletes the specified folder and all its contents', async () => {
      ...
      jest
        ...
        .mockResolvedValueOnce(data1 as never)
      jest
        .spyOn(service['s3Client'], 'send')
        .mockResolvedValueOnce(data2 as never)
      jest.spyOn(service['s3Client'], 'send').mockResolvedValue(data3 as never)
  ...
  describe('createMultiPart', () => {
    it('should return object multipart upload', async () => {
      ...
      const result = await service.createMultiPart('filename.txt', options)
  ...
  describe('completeMultipart', () => {
    ...
    it('should throw error when completing multipart error occurs', async () => {
      ...
      const result = service.completeMultipart('path/filename.txt', '12345', [
        { ETag: '1234', PartNumber: 1 },
        { ETag: '5678', PartNumber: 2 },
      ])
